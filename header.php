                            ...
                            <?php while (have_rows('menu', 'option')) : the_row(); ?>
                                <?php if (have_rows('cta_knoppen_menu')) : ?>
                                    <div class="ct-btns">
                                        ...
                                        <?php $i++; ?>
                                        <?php endwhile; ?>
                                        <ul <?php if ($i == 1) { ?>class="singular"<?php } ?>>
                                            <?php while (have_rows('cta_knoppen_menu')) : the_row(); ?>
                                                <?php $knop = get_sub_field('knop'); ?>
                                                <?php if ($knop) { ?>
                                                    <li><a class="menu-btn <?php if (get_sub_field('secondaire_kleur_gebruiken') == 1) { ?>secondair<?php } ?>" href="<?php echo $knop['url']; ?>" <?php if ($knop['target']) { ?>target="<?php echo $knop['target']; ?>" <?php } ?>><?php echo $knop['title']; ?></a></li>
                                                <?php } ?>
                                            <?php endwhile; ?>
                                        </ul>
                                    </div>
