<?php if ( have_rows( 'content' ) ): ?>
	<?php while ( have_rows( 'content' ) ) : the_row(); ?>
		...
		<?php elseif ( get_row_layout() == 'dienst_aanvragen_single' ) : ?>
			<section>
				<div class="container">
					<div class="row">
						<div class="col-md-6">
							<?php if ( have_rows( 'Tekstueel' ) ) : ?>
								<?php while ( have_rows( 'Tekstueel' ) ) : the_row(); ?>
									<h2 class="big"><?php the_sub_field( 'grote_titel' ); ?></h2>
									<?php the_sub_field( 'teksten' ); ?>
								<?php endwhile; ?>
							<?php endif; ?>
						</div>
						<div class="col-md-6">
							<div class="diens-aanvragen-single">
								<h2><?php the_sub_field( 'titel_formulier' ); ?></h2>
								<?php $formi = get_sub_field( 'formulier' ); 
									echo do_shortcode($formi);?>
							</div>
						</div>
					</div>
				</div>
								...
								<?php while ( have_rows( 'team_member' ) ) : the_row(); ?>
									<div class="team-member">
										<div class="info">
											<?php $afbeelding = get_sub_field( 'afbeelding' ); ?>
											<?php if ( $afbeelding ) { ?>
												<img src="<?php echo $afbeelding['sizes']['team']; ?>" alt="<?php echo $afbeelding['alt']; ?>" />
											<?php } ?>
											<h5><?php the_sub_field( 'naam' ); ?></h5>
											<span class="funtie"><?php the_sub_field( 'functie' ); ?></span>
										</div>
									</div>
								<?php endwhile; ?>
							<?php else : ?>
		...
		<?php elseif ( get_row_layout() == 'afwijkende_afbeelding' ) : ?>
			<section>
				<div class="container">
					<div class="row">
						<div class="col-md-12">
							<?php $afbeelding = get_sub_field( 'afbeelding' ); ?>
							<?php if ( $afbeelding ) { ?>
								<img style="transform: translateX(<?php the_sub_field( 'verplaatsing' ); ?>%);" src="<?php echo $afbeelding['url']; ?>" alt="<?php echo $afbeelding['alt']; ?>" />
							<?php } ?>
		...
		<?php elseif ( get_row_layout() == 'contact_mogelijkheden' ) : ?>
		<section>
			<div class="container">
				<div class="row">
					<div class="flexible">
						<?php if ( have_rows( 'telefoonnummers' ) ) : ?>
							<?php while ( have_rows( 'telefoonnummers' ) ) : the_row(); ?>
							<div class="tel-mogelijkheid">
								<img src="<?php echo get_template_directory_uri(); ?>/img/tel.svg"> <span class="tel-contact"><?php the_sub_field( 'telefoonnummer' ); ?></span> <span class="reactiesnelheid"><?php the_sub_field( 'bereikbaar_van_wanneer' ); ?></span>
							</div>
							<?php endwhile; ?>
						<?php else : ?>
							<?php // no rows found ?>
						<?php endif; ?>
						<?php if ( have_rows( 'emailadressen' ) ) : ?>
							<?php while ( have_rows( 'emailadressen' ) ) : the_row(); ?>
								<div class="mail-mogelijkheid">
									<img src="<?php echo get_template_directory_uri(); ?>/img/mail.svg"> <span class="mail-contact"><?php the_sub_field( 'emailadres' ); ?></span> <span class="reactiesnelheid"><?php the_sub_field( 'reactie_radius' ); ?></span>
								</div>
							<?php endwhile; ?>
						<?php else : ?>
							<?php // no rows found ?>
